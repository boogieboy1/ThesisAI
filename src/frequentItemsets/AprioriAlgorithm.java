...
public class AprioriAlgorithm {
	 ...
	 */
	public Itemsets runAlgorithm(double minsup, ArrayList<String> input, String output) throws IOException {
		...
		try {
			for(String row: input) { 
				...
				int[] transactionsInt = new int[transaction.length];
				...
				Arrays.sort(transactionsInt);
				database.add(transactionsInt);
...
//					System.out.println("test");
	 ...
	 void saveItemset(Itemset itemset) throws IOException {
		...
		if(writer != null){
			for(int i=0; i<itemset.size();i++){
				...
				String stringItem = intToString(item);
				writer.write(stringItem + " ");
			}
			writer.write(" #SUP: "
					+ (itemset.getAbsoluteSupport()*100)/databaseSize + "%");
			writer.newLine();
	...
	void saveItemsetToFile(Integer item, Integer support) throws IOException {
		...
		// if the result should be saved to a file
		if(writer != null){
			writer.write(stringItem + " #SUP: " + ((support*100)/databaseSize) + "%");
			writer.newLine();
		}// otherwise the result is kept into memory
		else{
