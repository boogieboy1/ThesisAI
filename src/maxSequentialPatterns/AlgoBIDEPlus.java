...
public class AlgoBIDEPlus {
	 ...
	 */
	public SequentialPatterns runAlgorithm(ResultSet rs, String outputPath, double minsupRelative) throws IOException {
		SequenceDatabase database = stringToIntDB(rs);
		// save minsup
	...
	private void bide(SequenceDatabase database, String outputFilePath) throws IOException{
		...
		for(Sequence sequence : database.getSequences()){
			...
			if(optimizedSequence.size() != 0){
				...
				initialDatabase.add(new PseudoSequenceBIDE(sequence, 0, 0));
			}else {
...
loop1:	for(Entry<Integer, Set<Integer>> entry : mapSequenceID.entrySet()){
	 ...
	 */
	private boolean checkBackwardExtension(SequentialPattern prefix, List<PseudoSequenceBIDE> projectedContext) {	
//		System.out.println("======" + prefix);
		...
		for(int i=0; i< totalOccurenceCount; i++){
			...
			Set<PseudoSequenceBIDE> alreadyVisitedSID = new HashSet<PseudoSequenceBIDE>();
...
//			int seqCount =0;
			...
			for(PseudoSequenceBIDE pseudoSequence : projectedContext){
				...
				int remainingSeqID = (projectedContext.size() - alreadyVisitedSID.size());
				if(highestSupportUntilNow != -1 && highestSupportUntilNow + remainingSeqID < projectedContext.size()) {
					break;
				}

				alreadyVisitedSID.add(pseudoSequence);
...
//				}
				...
				PseudoSequenceBIDE sequence = initialDatabase.get(pseudoSequence.getId());
				...
				if(period !=null){
					...
					Position currentCutPosition = new Position(pseudoSequence.firstItemset, pseudoSequence.firstItem);

					boolean hasBackwardExtension = findAllFrequentPairsForBackwardExtensionCheck(alreadyVisitedSID.size(), prefix, period, i, mapPaires, itemI, itemIm1, currentCutPosition);
...
//						System.out.println(prefix + " has a backward extension from " + i + "th maxperiod  in sequence from seq. " + sequenceID );
					...
					//===== OPTIMIZATION PART 2
					if((projectedContext.size() - alreadyVisitedSID.size()) < minsuppAbsolute){
...
//		totaltimeForBackwardExtension += System.currentTimeMillis() - start;
	 ...
	 */
	private boolean checkBackScanPruning(SequentialPattern prefix, List<PseudoSequenceBIDE> projectedContext) {	
...
//						// END NEW
			...
			for(PseudoSequenceBIDE pseudoSequence : projectedContext){
				...
				PseudoSequenceBIDE period = sequence.getIthSemiMaximumPeriodOfAPrefix(prefix.getItemsets(), i, currentCutPosition);
				
				if(period !=null){
//					// we add it to the list of maximum periods
					boolean hasExtension = findAllFrequentPairsForBackwardExtensionCheck(prefix.getAbsoluteSupport(), prefix, period, i, mapPaires, itemI, itemIm1, currentCutPosition);
					if(hasExtension) {
						return true;
					}
				}
	...
	protected boolean findAllFrequentPairsForBackwardExtensionCheck(
			int seqProcessedCount, SequentialPattern prefix,
			PseudoSequenceBIDE maximumPeriod, int iPeriod,
			Map<PairBIDE, PairBIDE> mapPairs, Integer itemI, Integer itemIm1, Position currentCutPosition) {

		int supportToMatch = prefix.getSequenceIDs().size();
		int periodItemsetCount = maximumPeriod.size();

		// for each itemset in that period
		for (int i = 0; i < periodItemsetCount; i++) {
			int itemCountOfIthItemset = maximumPeriod.getSizeOfItemsetAt(i);
			...
			// NEW march 20 2010 : check if I is after current position in
			// current itemset
			for (int j = 0; j < itemCountOfIthItemset; j++) {
			...
			for (int j = 0; j < itemCountOfIthItemset; j++) {
				...
				if (seqProcessedCount >= minsuppAbsolute) {
					// $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
					// normal case
					if (addPair(mapPairs, maximumPeriod.getId(), paire,
							supportToMatch)) {
						return true;
					}
					...
					if (sawIm1) {
						...
						if (addPair(mapPairs, maximumPeriod.getId(), paire2, supportToMatch)) {
							return true;
						}
					}
					...
					// normal case
					addPairWithoutCheck(mapPairs, maximumPeriod.getId(), paire);

					// // NEW: special cases
					if (sawIm1) {
						PairBIDE paire2 = new PairBIDE(isPrefix, !isPostfix,
								item);
						addPairWithoutCheck(mapPairs, maximumPeriod.getId(),
								paire2);
					}
...
//		if(prefix.size() == 2 && prefix.get(0).size() == 3  && prefix.get(1).size() == 3 &&
//				prefix.get(0).get(0) == 1 && prefix.get(0).get(1) == 3 && prefix.get(0).get(2) == 5
//				&& prefix.get(1).get(0) == 2 && prefix.get(1).get(1) == 3 && prefix.get(1).get(2) == 5){
//			System.out.println("DEBUGGING !!!!!!!! #### (1 3 5 )(2 3 5 )");
//		}
//		if(prefix.size() == 2 && prefix.get(0).size() == 3  && prefix.get(1).size() == 2 &&
//				prefix.get(0).get(0) == 1 && prefix.get(0).get(1) == 3 && prefix.get(0).get(2) == 5
//				&& prefix.get(1).get(0) == 2 && prefix.get(1).get(1) == 3 ){
...
//				debugProjectDBTime += System.currentTimeMillis() - start;
	...
	//			r.append('(');
				for(Integer item : itemset.getItems()){
					String string = intToString(item);
					r.append(string);
					r.append(' ');
				}
				r.append(" ** ");
	...
	public SequenceDatabase stringToIntDB(ResultSet input){
