...
public class PrefixSpanAlgorithm{
	...
	private int maximumPatternLength = 4;//Integer.MAX_VALUE;
	 ...
	 */
	public SequentialPatterns runAlgorithm(ArrayList <String> rs, double minsupRelative, String outputFilePath) throws IOException {
	...
	private void prefixSpan(SequenceDatabase database, String outputFilePath) throws IOException{
		...
		//29/02/2016: THIS IS NOT DONE AS WE NEED ALL THE ITEMS
		...
		for(Sequence sequence : database.getSequences()){
			...
			//if(optimizedSequence.size() != 0){
				...
				initialDatabase.add(new PseudoSequence(sequence, 0, 0));
			//}
		}
		...
		if(writer != null){
			...
			for(Itemset itemset : prefix.getItemsets()){
				...
				for(Integer item : itemset.getItems()){
					String itemString = intToString(item);
					r.append(itemString); // add the item
					r.append(' ');
				}
				r.append(" ** "); // add the itemset separator
			}		
			// add the support
			r.append("#SUP: ");
			...
			for(int i = 0; i< sequence.size(); i++){
				...
				if(index == sequence.getSizeOfItemsetAt(i)-1){ 
					...
					if ((i != sequence.size()-1)){
						...
						// add it to the projected database.
						sequenceDatabase.add(new PseudoSequence( sequence, i+1, 0));
						//System.out.println(sequence.getId() + "--> "+ sequence.toString());
//						break itemsetLoop;
					}
				}else{
					// create a new pseudo sequence and
...
//					break itemsetLoop;
	...
	private void recursion(SequentialPattern prefix, List<PseudoSequence> database, int k) throws IOException {	
		...
		// For each pair found (a pair is an item with a boolean indicating if it		// appears in an itemset that is cut (a postfix) or not, and the sequence IDs
	...
	protected Set<Pair> findAllFrequentPairs(List<PseudoSequence> sequences){
		...
		for(PseudoSequence sequence : sequences){
			...
			//for(int i=0; i< sequence.size(); i++){
				// for each item
				for(int j=0; j < sequence.getSizeOfItemsetAt(0); j++){
					Integer item = sequence.getItemAtInItemsetAt(j, 0);
					// create the pair corresponding to this item
					Pair pair = new Pair(sequence.isPostfix(0), item);   
	...
	private SequentialPattern appendItemToSequence(SequentialPattern prefix, Integer item) {
		...
		newPrefix.addItemset(new Itemset(item));  // nouvel itemset   + decalage
		return newPrefix;
	}
	...
	public SequenceDatabase stringToIntDB(ArrayList<String> input){
